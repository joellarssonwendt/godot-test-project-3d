...
var controller_sensitivity := 3.0  # For Xbox and other game controllers
...
func _process(delta: float) -> void:
	var right_stick_x = Input.get_joy_axis(0, JOY_AXIS_RIGHT_X)  # Right stick X
	var right_stick_y = Input.get_joy_axis(0, JOY_AXIS_RIGHT_Y)  # Right stick Y

	# Apply deadzone to prevent accidental movement
	var deadzone = 0.2
	...
	# Apply sensitivity and assign to camera direction
	_camera_input_direction.x = -right_stick_x * controller_sensitivity
	_camera_input_direction.y = right_stick_y * controller_sensitivity  # Invert Y for natural movement
...
func _physics_process(delta: float) -> void:
	...
	var raw_input := Input.get_vector("left", "right", "forward", "back", 0.4)
